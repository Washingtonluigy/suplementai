...
// O token fica exclusivamente nas Environment Variables do Netlify.
// Assim ele não entra no bundle público nem precisa ser commitado no Git.
const token = () => String(process.env.FRENET_TOKEN || '').trim();
...
async function quote(input) {
  ...
  if (!authToken) return json(503, { error: 'Token Frenet não configurado.' });
...
export async function handler(event) {
  ...
  try {
    if (event.httpMethod === 'GET') return json(200, { configured: Boolean(token()), provider: 'frenet', endpoint: FRENET_URL });
    if (event.httpMethod !== 'POST') return json(405, { error: 'Método não permitido.' });
    const input = JSON.parse(event.body || '{}');
    const action = String(input?.action || 'quote');
    if (action === 'quote') return await quote(input);
    if (action === 'status') return json(200, { configured: Boolean(token()), provider: 'frenet', endpoint: FRENET_URL });
