...
export async function handler(event) {
  ...
  try {
    ...
    if (action === 'test_config') {
      return json(200, {
        ok: true,
        configured: Boolean(integration.client_id && integration.store_id && integration.food99?.authToken),
    ...
    const authToken = String(integration.food99?.authToken || '').trim();
    if (!authToken) return json(400, { ok: false, error: 'Cole o token de autenticação da loja de teste 99Food e salve antes de enviar o item.' });
