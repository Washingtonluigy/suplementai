...
function quoteDetails(data) {
    ...
    const find = (node, keys, depth = 0) => {
        if (depth > 7 || node == null) return null;
        if (Array.isArray(node)) { for (const item of node) { const value = find(item, keys, depth + 1); if (value != null) return value; } return null; }
        if (typeof node !== 'object') return null;
        for (const key of keys) if (Object.prototype.hasOwnProperty.call(node, key)) { const value = toNumber(node[key]); if (value != null) return value; }
        for (const value of Object.values(node)) if (value && typeof value === 'object') { const nested = find(value, keys, depth + 1); if (nested != null) return nested; }
        return null;
    };
    return {
        price: find(data, ['estimativa_valor','valor_estimativa','valor_estimado','valor_total','valor','preco','price','taxa','custo']),
        minutes: find(data, ['estimativa_minutos','estimativa_tempo_minutos','tempo_minutos','minutos','tempo']),
        km: find(data, ['estimativa_km','km','distancia_km','distancia']),
    };
}
...
export default function MtDeliveryModal({ order, franchiseId, onClose, onSuccess }) {
    ...
    const loadQuote = async () => {
        ...
        try {
            ...
            const parsed = quoteDetails(data);
            if (parsed.price == null) throw new Error('A MT respondeu, mas não retornou o valor da estimativa.');
            if (Number(parsed.price) <= 0) throw new Error('A MT retornou taxa R$ 0,00. Confira endereço, categoria e tabela de tarifas antes de solicitar.');
            setQuote(data);
        }
        catch (err) {
