...
import { quoteExistingMt, customerDeliveryPortion, fallbackDeliveryFee, haversineKm } from '/app/lib/mtClient.js';
...
export default function PublicStore() {
    ...
    const fallbackDistance = store?.delivery ? haversineKm(store.delivery.latitude, store.delivery.longitude, selectedLat, selectedLng) : null;
    const staticRawFee = fallbackDeliveryFee(store?.delivery ?? null, { neighborhood: form.neighborhood, distanceKm: fallbackDistance });
    const rawDeliveryFee = mtQuote?.fee ?? staticRawFee;
    const deliveryFee = roundMoney(form.orderMode === 'delivery' ? customerDeliveryPortion(rawDeliveryFee, storePercent) : 0);
    ...
    useEffect(() => {
        ...
        const timer = window.setTimeout(async () => {
            ...
            setMtQuote(null);
            try {
                const quote = await quoteExistingMt(store.delivery, {
