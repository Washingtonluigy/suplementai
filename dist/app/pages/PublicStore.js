...
export default function PublicStore() {
    ...
    const [mtQuote, setMtQuote] = useState(null);
    const [mtQuoteLoading, setMtQuoteLoading] = useState(false);
    const [mtQuoteError, setMtQuoteError] = useState('');
    const [addressSuggestions, setAddressSuggestions] = useState([]);
    const [addressSearchLoading, setAddressSearchLoading] = useState(false);
    const [form, setForm] = useState({
        name: '', phone: '', address: '', deliveryNumber: '', deliveryNeighborhood: '', deliveryCity: '', deliveryState: '', reference: '', notes: '',
        orderMode: 'delivery',
    ...
    useEffect(() => {
        const loadStore = async () => {
            ...
            const [{ data: categoryData }, { data: productData }, { data: deliveryData }, { data: promoData }] = await Promise.all([
                ...
                supabase.from('delivery_settings').select('id,franchise_id,enabled,contact_phone,contact_whatsapp,address,latitude,longitude,fee_type,fee_value,fee_ranges,neighborhood_fees,fee_store_percent,mt_entregas_enabled,mt_entregas_category_id,created_at').eq('franchise_id', franchise.id).maybeSingle(),
    ...
    const mtEnabled = Boolean(form.orderMode === 'delivery' && store?.delivery?.enabled && store?.delivery?.mt_entregas_enabled);
    const storePercent = Math.min(100, Math.max(0, store?.delivery?.fee_store_percent ?? 0));
    const normalizeAddressPart = (value) => value.trim().toLocaleLowerCase('pt-BR').normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    const staticNeighborhoodFee = form.orderMode === 'delivery' && !mtEnabled && store?.delivery?.fee_type === 'neighborhood'
        ? Number((store.delivery.neighborhood_fees ?? []).find((item) => normalizeAddressPart(String(item?.name ?? '')) === normalizeAddressPart(form.deliveryNeighborhood))?.fee ?? 0)
        : 0;
    // Com MT ativa, a taxa vem da estimativa real da Machine. A ajuda de custo da loja
    // reduz apenas a parcela cobrada do cliente.
    const baseDeliveryFee = mtEnabled ? Number(mtQuote?.rawFee ?? 0) : staticNeighborhoodFee;
    const customerFeePortion = mtEnabled ? Number(mtQuote?.customerFee ?? 0) : baseDeliveryFee * (1 - storePercent / 100);
    const deliveryFee = roundMoney(form.orderMode === 'delivery' ? Math.max(0, customerFeePortion) : 0);
    const total = roundMoney(Math.max(0, subtotal - couponDiscount - pixDiscount + deliveryFee));
    // Calcula automaticamente quando o cliente termina de digitar o endereço completo.
    // O endpoint v2 exige rua/número, bairro, cidade e UF do destino.
    useEffect(() => {
        if (!store || !mtEnabled) {
            setMtQuote(null);
            setMtQuoteError('');
        ...
        const number = form.deliveryNumber.trim();
        const address = [street, number].filter(Boolean).join(', ');
        const neighborhood = form.deliveryNeighborhood.trim();
        const city = form.deliveryCity.trim();
        const state = form.deliveryState.trim().toUpperCase();
        if (street.length < 3 || number.length < 1 || neighborhood.length < 2 || city.length < 2 || state.length !== 2) {
            setMtQuote(null);
            setMtQuoteError('');
            setMtQuoteLoading(false);
            return;
        ...
        const timer = window.setTimeout(async () => {
            ...
            try {
                const response = await fetch('/.netlify/functions/mt-entregas', {
                    ...
                    body: JSON.stringify({ action: 'quote', franchise_id: store.franchise.id, payload: {
                            delivery_address: address, delivery_neighborhood: neighborhood,
                            delivery_city: city, delivery_state: state,
                            category_id: store.delivery?.mt_entregas_category_id ?? undefined,
                        } }),
                });
                const data = await response.json().catch(() => ({}));
                if (!response.ok || data?.success === false)
                    throw new Error(data?.error || data?.errors?.join?.(', ') || 'A MT Entregas não conseguiu calcular este endereço.');
                const source = data?.data ?? data;
                const rawFee = Number(source?.estimativa_valor);
                if (!Number.isFinite(rawFee) || rawFee < 0)
                    throw new Error('A MT Entregas não retornou um valor válido para a entrega.');
                const customerFee = roundMoney(rawFee * (1 - storePercent / 100));
                setMtQuote({ rawFee: roundMoney(rawFee), customerFee, km: Number.isFinite(Number(source?.estimativa_km)) ? Number(source.estimativa_km) : null, minutes: Number.isFinite(Number(source?.estimativa_minutos)) ? Number(source.estimativa_minutos) : null });
            }
            catch (err) {
                if (err?.name !== 'AbortError')
                    setMtQuoteError(err?.message || 'Não foi possível calcular a taxa de entrega.');
            }
            finally {
    ...
    }, [store, mtEnabled, form.address, form.deliveryNumber, form.deliveryNeighborhood, form.deliveryCity, form.deliveryState, storePercent]);
    // Sugestões de endereço: a busca começa com 3 caracteres. O ViaCEP fornece logradouro/bairro/cidade/UF;
    // depois que o cliente informa o número, a taxa é calculada pela API real da MT Entregas.
    useEffect(() => {
        if (!store || !mtEnabled || form.orderMode !== 'delivery') {
        ...
        const timer = window.setTimeout(async () => {
            ...
            try {
                const response = await fetch('/.netlify/functions/mt-entregas', {
                    method: 'POST', headers: { 'Content-Type': 'application/json' }, signal: controller.signal,
                    body: JSON.stringify({ action: 'address_search', franchise_id: store.franchise.id, payload: { query, city: form.deliveryCity, state: form.deliveryState } }),
                });
                const data = await response.json().catch(() => ({}));
                if (!response.ok || data?.success === false)
                    throw new Error(data?.error || 'Falha ao localizar o endereço.');
                setAddressSuggestions(Array.isArray(data?.data) ? data.data : []);
            }
            catch (err) {
                if (err?.name !== 'AbortError')
                    setAddressSuggestions([]);
        ...
        return () => { window.clearTimeout(timer); controller.abort(); };
    }, [store, mtEnabled, form.orderMode, form.address, form.deliveryCity, form.deliveryState]);
    ...
    const submitOrder = async (event) => {
        ...
        if (!store || cart.length === 0)
            return;
        setSubmitError('');
        if (form.orderMode === 'delivery' && mtEnabled && (!mtQuote || mtQuoteLoading)) {
            setSubmitError(mtQuoteLoading ? 'Aguarde a MT Entregas terminar o cálculo da taxa.' : (mtQuoteError || 'Preencha o endereço completo e aguarde a taxa da MT Entregas antes de finalizar.'));
            return;
        }
        setSubmitting(true);
        const orderData = {
            ...
            address: form.orderMode === 'delivery' ? [[form.address.trim(), form.deliveryNumber.trim()].filter(Boolean).join(', '), form.deliveryNeighborhood.trim(), `${form.deliveryCity.trim()}/${form.deliveryState.trim().toUpperCase()}`].filter(Boolean).join(', ') : null,
        ...
        if (form.paymentChannel === 'store') {
            ...
            setAppliedCoupon(null);
            setCouponCode('');
            setForm({ name: '', phone: '', address: '', deliveryNumber: '', deliveryNeighborhood: '', deliveryCity: '', deliveryState: '', reference: '', notes: '', orderMode: 'delivery', paymentChannel: 'store', storePaymentMethod: 'cash', onlinePaymentMethod: 'pix', customerEmail: '', customerCpf: '' });
            setSubmitting(false);
            return;
        }
