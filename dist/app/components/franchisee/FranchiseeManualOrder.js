...
import { quoteExistingMt, customerDeliveryPortion } from '/app/lib/mtClient.js';
...
export default function FranchiseeManualOrder({ franchiseId }) {
    ...
    const [deliveryDetails, setDeliveryDetails] = useState({ phone: '', address: '', number: '', neighborhood: '', city: '', state: '', reference: '' });
    const [manualMtQuote, setManualMtQuote] = useState(null);
    const [manualMtLoading, setManualMtLoading] = useState(false);
    const [manualMtError, setManualMtError] = useState('');
    ...
    useEffect(() => {
        ...
        supabase.from('delivery_settings').select('*').eq('franchise_id', franchiseId).maybeSingle().then(({ data }) => setDeliverySettings(data ?? null));
    }, [franchiseId]);
    useEffect(() => {
        if (user) {
    ...
    useEffect(() => {
        ...
        const timer = window.setTimeout(async () => {
            ...
            setManualMtQuote(null);
            try {
                const quote = await quoteExistingMt(deliverySettings, {
                ...
                setDeliveryFee(String(customerDeliveryPortion(quote.fee, deliverySettings.fee_store_percent ?? 0)));
